...
class widget extends MY_Controller {
	...
	{
		$data['konten'] = $this->load->view('widget/tambah', NULL, TRUE);
	...
	{
		$item = 
		$data['konten'] = $this->load->view('widget/daftar', NULL, TRUE);
		$this->load->view ('main', $data);
	}

	public function detail()
	{
		$data['konten'] = $this->load->view('widget/detail', NULL, TRUE);
		$this->load->view ('main', $data);
	}

	public function ubah()
	{
		$data['konten'] = $this->load->view('widget/ubah', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	public function simpan_ubah() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$get_old_file = $this->db->query("SELECT * FROM widget where id='$id'");
			...
			$this->db->update("widget", $data, "id = " . $id);
	...
	public function hapus() {      
		$key = $_REQUEST['key'];
		$this->db->delete('widget', array('id' => $key));
		echo '<div class="alert alert-success">Data Widget sudah dihapus.</div>';    
	}
}
