...
<div class="main-content" style="margin-top: 20px;">		
	<div class="col-md-9 total-news">
		<div class="posts">
			...
			<div style="max-width:100%; max-height: 100%;height: 80%;">
				<header>
					<div class="title-content">
						<button id="last" class="btn btn-block btn-success btn-lg" style="background: #448D00;"></button>
						<span style="float: left;">
							<h1 id="month" style="padding-right: "></h1>
							<h2 id="year"  style="padding-right: "></h2>
						</span>
						<button id="next" style="background:#448D00;" class="btn btn-block btn-success btn-lg" ></button>
					</div>
				</header>
	...
	<div class="col-md-3 side-bar" style="margin-bottoms: 1000px;">
		<div class="clearfix"></div>
		<div class="clearfix"></div>
		<?php
														...
														.ci-cal-container .ci-body .ci-row .ci-event-row:nth-child(1) {
															...
															.ci-cal-container .ci-body .ci-row .ci-event-row:nth-child(2) {
																...
																.ci-cal-container .ci-body .ci-row>div.ci-today {
																	background: rgba(55, 157, 129, 0.42);
																}
...
<?php 
$get_agenda = $this->db->get('agenda');
foreach($get_agenda->result() as $daftar){
	$tanggal_mulai = explode("-",$daftar->tanggal_mulai);
	$tanggal_selesai = explode("-",$daftar->tanggal_selesai);
	?>
	...
	event = {
		...
		details: {
			...
			title: "<a href='<?php echo base_url().'web/detail_agenda/'.$daftar->id; ?>'><?php echo $daftar->judul_kegiatan; ?></a>",
			location: '',
			country: '',
			city: '',
			public: true
		...
		start: {
			...
			time: '<?php echo $daftar->jam_mulai ?>',
			month: <?php echo $tanggal_mulai[1] ?>,
			day: <?php echo $tanggal_mulai[2] ?>,
			year: <?php echo $tanggal_mulai[0] ?>,

		},

		end: {

			time: '<?php echo $daftar->jam_selesai ?>',
			month: <?php echo $tanggal_selesai[1] ?>,
			day: <?php echo $tanggal_selesai[2] ?>,
			year: <?php echo $tanggal_selesai[0] ?>,
